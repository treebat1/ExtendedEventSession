...
SELECT  DATEADD(mi, DATEDIFF(mi, GETUTCDATE(), CURRENT_TIMESTAMP), x.event_data.value('(event/@timestamp)[1]', 'datetime2')) AS event_time,
       ...
       x.event_data
FROM    sys.fn_xe_file_target_read_file ('c:\ExtendedEvents\StoredProcedureDurationPier1_0_131202780951590000.xel', null, null, null)
...
FROM   xevents AS x
WHERE x.event_type = 'rpc_completed';
...
--select top 100 *, LEFT(STATEMENT,24)
--from query_stats
--where event_interval = '2016/10/07 00:00:00'

select *, duration_ms/NULLIF(row_count,0) AS MSPerRow, LEFT(STATEMENT,24)
from query_stats
where event_interval between '2016/10/06 19:12:00' and '2016/10/07 07:12:00'
and duration_ms <= 1000000
and LEFT(STATEMENT,24) = 'exec dbo.Get_Subject_Pro'